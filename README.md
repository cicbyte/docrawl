...
> 将任意文档网站（VitePress、Docusaurus、GitBook 等）批量转换为结构化 Markdown 的 CLI 工具。
...
docrawl 通过 ChromeDP 渲染页面、自动识别目录结构、智能提取正文，并支持 AI 辅助分析提升抓取质量。

## 功能特性

- **智能扫描** — 自动检测文档站点的目录结构和正文区域，支持展开折叠项
- **AI 分析** — 使用 AI 分析页面截图，识别最佳内容选择器
- **并发抓取** — 多 worker 并发抓取，实时进度条显示
- **请求延迟** — 支持固定或随机请求间隔，避免对目标站点造成压力
- **Markdown 转换** — 完整保留代码块、表格、链接、图片，自动生成 YAML Front Matter
- **选择器验证** — verify 命令支持调试 CSS/XPath 选择器
- **优雅退出** — Ctrl+C 安全中断，不丢失已抓取内容
...
> 注意：`go install` 不注入版本信息，版本号会显示为 `dev`。如需完整版本信息，请下载 [Release](https://github.com/cicbyte/docrawl/releases) 中的二进制文件。
...
- AI API 密钥（可选，默认使用智谱 AI GLM-4-Flash）

## 快速开始

```bash
# 1. 扫描文档站点，生成 catalog.json
docrawl scan https://docs.golang.org

# 2. 根据 catalog.json 抓取内容
...
自动识别目录结构，生成 `catalog.json` 供 fetch 使用。
...
docrawl fetch -i catalog.json -o ./output -d 1,3        # 请求间随机等待 1~3 秒
docrawl fetch -i catalog.json -o ./output -d 2          # 请求间固定等待 2 秒
```
